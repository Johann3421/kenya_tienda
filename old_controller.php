...
class ProductoController extends Controller
{
    ...
    public function inicio()
    {
        ...
        $productos = Producto::with('especificaciones')->paginate(10); // Ajusta seg├║n necesites
...
public function store(Request $request)
{
    ...
    try {
        ...
        return [
            'type'     =>  'danger',
            'title'    =>  'ERROR: ',
            'message'  =>  $th . ' Ocurri├│ un error al guardar el Producto, intente nuevamente o contacte al Administrador del Sistema.'
        ];
    }
}
...
public function update(Request $request)
{
    ...
    try {
        ...
        return [
            'type'     =>  'danger',
            'title'    =>  'ERROR: ',
            'message'  =>  $th . ' Ocurri├│ un error al actualizar el Producto, intente nuevamente o contacte al Administrador del Sistema.'
        ];
    }
}

/**
 * Sube el PDF de ficha t├®cnica a storage/app/public/pdfs/{producto_id}
...
public function subirFichaTecnica(Request $request, $producto)
{
    if ($request->hasFile('pdf_ficha')) {
        ...
        $extension = $file->extension();
        // El nombre ser├í: soporte_{id}_{random}.pdf
    ...
    public function web(Request $request)
    {
        ...
        return [
            'type'          =>  'success',
            'title'         =>  'CORRECTO: ',
            'message'       =>  'La Categor├¡a se guardo correctamente.'
        ];
    }
    ...
    public function detalle($id)
    {
        ...
        // Orden can├│nico de specs (cubre monitores y PCs)
        $specPriority = [
            // ÔöÇÔöÇ Monitores ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            'Tama├▒o de Pantalla'          => 1,
            'Tecnolog├¡a de Pantalla'      => 2,
            'Panel'                       => 3,
            'Relaci├│n de Aspecto'         => 4,
            'Resoluci├│n'                  => 5,
            'Contraste'                   => 6,
            'Brillo'                      => 7,
            '├üngulo de Visi├│n'            => 8,
            ...
            'DisplayPort'                 => 15,
            'Alimentaci├│n'                => 16,
            'Garant├¡a de F├íbrica'         => 17,
            'Empaque'                     => 18,
            'Accesorios'                  => 19,
            'Caracter├¡sticas Adicionales' => 20,
            // ÔöÇÔöÇ PCs ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            'Procesador'                  => 30,
            'RAM'                         => 31,
            'Almacenamiento'              => 32,
            'Gr├íficos'                    => 33,
        ...
        // Resumen para la parte superior (4 campos m├ís relevantes)
        // Monitores: Tama├▒o ÔåÆ Panel ÔåÆ Salidas ÔåÆ Garant├¡a
        // PCs: los primeros 4 del orden ya definido
        $isMonitor = $especificaciones->firstWhere('campo', 'Tama├▒o de Pantalla') !== null;
        if ($isMonitor) {
            $resumen = collect();
            foreach (['Tama├▒o de Pantalla', 'Panel'] as $campo) {
            ...
            $f = $especificaciones->firstWhere('campo', 'Garant├¡a de F├íbrica');
            if ($f) $resumen->push($f);
    ...
    private function syncSpecs($producto) {
        $fields = [
            ...
            'Gr├íficos' => 'tarjetavideo',
            'Sistema Operativo' => 'sistema_operativo',
            'Suite Ofim├ítica' => 'suite_ofimatica',
            'Garant├¡a de F├íbrica' => 'garantia_de_fabrica',
            'Empaque' => 'empaque_de_fabrica',
            'Certificaciones' => 'certificacion',
            'Lan' => 'conectividad',
            'Wlan' => 'conectividad_wlan',
            'Puertos M├¡nimos' => 'conectividad_usb',
            'Sonido' => 'sonido',
            'Chipset' => 'chipset',
            'Slot de Expansi├│n' => 'slot_expansion',
    ...
    public function agregarEspecificacion(Request $request, $productoId)
    {
        ...
        return back()->with('success', 'Especificaci├│n agregada correctamente');
    }
    ...
    public function importarEspecificacionesExcel(Request $request, $productoId)
    {
        ...
        try {
            ...
            // Versi├│n compatible con Laravel Excel 3.1
    ...
    public function asignarFiltrosGenerico(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'productos.*' => ['required', 'json', function ($attribute, $value, $fail) {
                ...
                if (!is_array($decoded)) {
                    $fail('El formato de los filtros no es v├ílido.');
                }
            }],
        ]);
