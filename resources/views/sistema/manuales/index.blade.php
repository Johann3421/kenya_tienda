            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10" style="color: #6b6b6b;"><i class="fas fa-truck-loading"></i>Manuales</h5>
                    ...
                    <div class="modal fade" id="formularioModal" tabindex="-1" role="dialog" data-backdrop="static"
                        ...
                        <div class="modal-dialog" :class="modal_size" role="document">
                            ...
                            <div class="modal-content" v-if="methods == 'create'">
                                <div class="modal-header" style="padding: 10px 15px">
                                    <h5 class="mb-0">NUEVO <span
                                            style="color: #929292; font-size: 17px; font-weight: 400;">(MANUAL)</span></h5>
                                ...
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <div class="form-row">
                                        <div class="form-group col-md-12 form_producto">
                                            ...
                                            <ul v-if="productos.length !== 0 && search_producto.length !== 0"
                                                ...
                                                <li v-for="producto in productos" :value="producto.id"
                                                    class="item_producto" v-on:click="Fila_producto(producto.id, producto)">
                                                    @{{ producto.nombre }}</li>
                                            </ul>
                                        </div>
                                        ...
                                        <div class="form-group col-md-2">
                                            <label for="pdf_manual" class="label-sm">MANUAL</label>
                                            <label class="image" for="file" title="Buscar Imagen">
                                                PDF
                                                <br>
                                                <i class="fa fa-plus-circle"></i>
                                                <input type="file" id="file" style="display: none;"
                                                    v-on:change="changePdf($event)" accept="PDF-MANUALES/*">
                                            </label>
                                            <small class="form-text error-color"
                                                v-if="errors.pdf_manual">@{{ errors.pdf_ficha[0] }}</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer" style="padding: 10px 15px;">
                                    <button class="btn btn-primary btn-block event-btn" v-on:click="Store"
                                        ...
                                        <span class="btn-text" v-if="!loading"><i class="feather icon-times"></i>
                                            Guardar</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="modal-content" v-if="methods == 'edit'">
                                <div class="modal-header" style="padding: 10px 15px">
                                    <h5 class="mb-0">EDITAR <span
                                            style="color: #929292; font-size: 17px; font-weight: 400;">(MANUAL)</span>
                                    </h5>
                                    <button type="button" title="Cerrar" data-dismiss="modal" aria-label="Close"
                                        v-on:click="closeModal(methods)" class="btn btn-danger btn-xs float-right"
                                        style="padding: 0px 7px;">X</button>
                                ...
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <div class="form-row">
                                        <div class="form-group col-md-12 form_producto">
                                            ...
                                            <input type="hidden" id="producto_id_actualizar" v-model="producto_id_actualizar" class="form-control" :class="[errors.categoria_id_actualizar ? 'is-invalid' : '']" :readonly="loading">
                                            ...
                                            <ul v-if="productos.length !== 0 && search_producto.length !== 0"
                                                v-show="mostrar" class="listProductos">
                                                <li v-for="producto in productos" :value="producto.id"
                                                    class="item_producto" v-on:click="Fila_producto(producto.id, producto, 'edit')">
                                                    @{{ producto.nombre }}</li>
                                            </ul>
                                        </div>
                                        <div class="form-group col-md-12">
                            ...
                            <div class="modal-content" v-if="methods == 'delete'">
                                ...
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <p class="text-center">
                                        Realmente desea eliminar el Driver <strong>"@{{ seleccion.descripcion }}"</strong>
                                    </p>
                                </div>
                                <div class="modal-footer" style="padding: 10px 15px;">
                        ...
                        <div class="mb-3 mt-3 col-md-3">
                            <div class="p-b-10">
                                <input type="text" class="form-control" id="seacrh" v-model="search"
                                    placeholder="Descripcion de manual" v-on:keyup.enter="Buscar">
                            </div>
                        ...
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="cell-3 text-center">Descripción</th>
                                    <th class="cell-4 text-center">Estado</th>
                                    <th class="cell-5 text-center"><i class="fas fa-solid fa-screwdriver"></i><i
                                            class="fas fa-solid fa-download"></i>Descargar</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr id="list-loading">
                                ...
                                <template v-if="listTable">
                                    <template v-if="listaRequest.length != 0">
                                        <tr v-for="(manual, index) in listaRequest"
                                            :class="{ activado: active == manual.id }"
                                            v-on:click="Fila_manual(manual.id, manual)" style="cursor: pointer;">
                                            <td class="text-center">@{{ (index + pagination.index + 1) }}</td>
                                            <td class="displaynone">
                                                @{{ manual.producto_id }}
                                            </td>
                                            <td>
                                                @{{ manual.producto_descripcion }}
                                            </td>
...
@section('js')
    <script src="{{ asset('js/views/manual/manual.js') }}"></script>
