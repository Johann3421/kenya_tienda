...
@section('app-name')
<title>KENYA - DRVIER - drivers_ruta</title>
@endsection
@section('css')
<style>
    .activado {
        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="page-header-title">
                    <h5 class="m-b-10" style="color: #6b6b6b;"><img src="//icons.veryicon.com/png/128/object/material-design-icons-1/folder-google-drive.png" srcset="//icons.veryicon.com/png/256/object/material-design-icons-1/folder-google-drive.png 2x" alt="folder-google-drive Icon" style="width: 30px; height: 30px"> DRIVERS</h5>
                </div>
                <ul class="breadcrumb" style="font-size: 15px;">
                    ...
                    <li class="breadcrumb-item"><a href="../">RUTAS - DRIVERS</a></li>
                </ul>
            </div>
        </div>
    </div>
</div>
<div class="row" style="top: 40px; position: inherit;" id="form-driver_ruta">
                    ...
                    <div class="modal-dialog" :class="modal_size" role="document">
                        ...
                        <div class="modal-content" v-if="methods == 'create'">
                            <div class="modal-header" style="padding: 10px 15px">
                                <h5 class="mb-0">NUEVO <span style="color: #929292; font-size: 17px; font-weight: 400;">(RUTA _ DRIVER)</span>
                                </h5>
                                ...
                                <div class="form-row">
                                    <div class="form-group col-lg-6">
                                        <label class="image" for="file" title="Buscar Imagen">
                                            ...
                                            <input type="file" id="file" style="display: none;" v-on:change="changeZip($event)" accept="DRIVERS/*">
                                        </label>
                                        <small class="form-text error-color" v-if="errors.pdf_driver">@{{ errors.pdf_driver[0] }}</small>
                                    </div>
                                </div>
                            </div>
            ...
            <div
                ...
                v-text="uploadProgress + '%'">
                        ...
                        <div class="modal-content" v-if="methods == 'edit'">
                            <div class="modal-header" style="padding: 10px 15px">
                                <h5 class="mb-0">EDITAR <span style="color: #929292; font-size: 17px; font-weight: 400;">(RUTA_ DRIVER)</span>
                                </h5>
                                <button type="button" title="Cerrar" data-dismiss="modal" aria-label="Close" v-on:click="closeModal(methods)" class="btn btn-danger btn-xs float-right" style="padding: 0px 7px;">X</button>
                            </div>
                            <div class="modal-body" style="padding: 15px 15px;">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        <label for="nombre_driver" class="label-sm">NOMBRE DE LA DRIVER</label>
                                        <input type="text" id="nombre_driver" v-model="nombre_driver" class="form-control" :class="[errors.nombre_driver ? 'is-invalid' : '']" :readonly="loading">
                                        <small class="form-text error-color" v-if="errors.nombre_driver">@{{ errors.nombre_driver[0] }}</small>
                                    </div>
                                </div>
                                <div class="form-row">
        ...
        <div
            ...
            v-text="uploadProgress + '%'">
                        ...
                        <div class="modal-content" v-if="methods == 'delete'">
                            <div class="modal-header" style="padding: 10px 15px">
                                <h5 class="mb-0">ELIMINAR <span style="color: #929292; font-size: 17px; font-weight: 400;">(DRIVER)</span></h5>
                                <button type="button" title="Cerrar" data-dismiss="modal" aria-label="Close" v-on:click="closeModal(methods)" class="btn btn-danger btn-xs float-right" style="padding: 0px 7px;">X</button>
                            </div>
                            <div class="modal-body" style="padding: 15px 15px;">
                                <p class="text-center">
                                    Realmente desea eliminar el Driver <strong>"@{{ seleccion.rute }}"</strong>
                                </p>
                            </div>
                            <div class="modal-footer" style="padding: 10px 15px;">
                ...
                <div class="row">
                    <div class="mb-3 mt-3 col-md-9">
                        <button type="button" class="btn btn-icon btn-primary mr-2" style="min-width: 88px;" data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('', null, 'create', null)">
                            <div style="font-size: 30px;"><i class="fas fa-plus"></i></div>
                            <div>Nuevo</div>
                        </button>

                        <button type="button" class="btn btn-icon btn-info mr-2" style="min-width: 88px;" v-if="active != 0" data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('', active, 'edit', seleccion)">
                        ...
                        <button type="button" class="btn btn-icon btn-danger mr-2" style="min-width: 88px;" v-if="(active != 0)" data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('modal-sm', active, 'delete', seleccion.garantia)">
                    ...
                    <div class="mb-3 mt-3 col-md-3">
                        <div class="p-b-10">
                            <input type="text" class="form-control" id="seacrh" v-model="search" placeholder="Descripcion de Garantia" v-on:keyup.enter="Buscar">
                        </div>
                    ...
                    <table class="table table-sm">
                        <thead>
                            <tr>
                                ...
                                <th class="cell-2">Nombre de la Driver</th>
                                <th class="cell-2">DRIVER</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr id="list-loading">
                            ...
                            <template v-if="listTable">
                                <template v-if="listaRequest.length != 0">
                                    <tr v-for="(driver_ruta, index) in listaRequest" :class="{ activado: active == driver_ruta.id }" v-on:click="Fila_driver_ruta(driver_ruta.id, driver_ruta)" style="cursor: pointer;">
                                        <td class="text-center">@{{ (index + pagination.index + 1) }}</td>
                                        <td class="displaynone">
                                            @{{ driver_ruta.id }}
                                        </td>
                                        <td>
                                            @{{ driver_ruta.nombre_driver }}
                                        </td>
                                        <td id="p1">
                                            @{{ driver_ruta.rute }}
                                        </td>
                    ...
                    <div class="col-sm-4">
                        <nav class="text-center" aria-label="...">
                            <ul class="pagination" style="justify-content: center;">
                                <a href="#" v-if="pagination.current_page > 1" class="pag-inicio-fin" title="Página inicio" v-on:click.prevent="changePage(1)">
