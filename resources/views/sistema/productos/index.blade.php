                ...
                <div class="card-body">
                    ...
                    <div class="modal fade" id="formularioModal" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
                        <div class="modal-dialog" :class="modal_size" role="document">
                            ...
                            {{-- NUEVO --}}
                            {{-- MODAL PARA AGREGAR ESPECIFICACIONES --}}
...
<div class="modal-content" v-if="methods == 'add_spec'">
    <div class="modal-header" style="padding: 10px 15px">
        ...
        <button type="button" class="btn btn-danger btn-xs float-right"
                ...
                v-on:click="closeModal(methods)" style="padding: 0px 7px;">X</button>
    </div>
            ...
            <div class="mb-4">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover">
                        ...
                        <tbody>
                            <tr v-for="(espec, index) in seleccion.especificaciones" :key="index" :data-id="espec.id">
                                <td class="editable-cell" data-campo="campo">@{{espec.campo}}</td>
                                <td class="editable-cell" data-campo="descripcion">@{{espec.descripcion}}</td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-xs btn-danger btn-eliminar-espec" :data-id="espec.id">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </td>
                            </tr>

                            <tr v-if="!seleccion.especificaciones || seleccion.especificaciones.length == 0">
                                <td colspan="3" class="text-center text-muted">No hay especificaciones registradas</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<div class="modal-content" v-if="methods == 'import_spec'">
    ...
    <form :action="'/producto/' + seleccion.id + '/especificaciones/import'" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="modal-body" style="padding: 15px 15px;">
            <div class="form-group">
            ...
            modal.on('shown.bs.modal', function () {
                ...
                modal.off('click', '.btn-eliminar-espec').on('click', '.btn-eliminar-espec', function () {
                    ...
                    axios.delete(`/producto/especificaciones/${especId}`)
                        .then(() => {
                            vueApp.seleccion.especificaciones = vueApp.seleccion.especificaciones.filter(e => e.id != especId);
                        });
                });
                    ...
                    const input = $('<input type="text" class="form-control form-control-sm">')
                        ...
                        .on('blur keyup', function (e) {
                            if (e.type === 'blur' || e.key === 'Enter') {
                                ...
                                const data = { [campo]: nuevoValor };
                                axios.put(`/producto/especificaciones/${id}/editar`, data)
                                    .then(() => {
                                        td.text(nuevoValor);
                                        const espec = vueApp.seleccion.especificaciones.find(e => e.id == id);
