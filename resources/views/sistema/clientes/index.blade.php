...
@section('content')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10" style="color: #6b6b6b;"><i class="fas fa-truck-loading"></i> Clientes</h5>
                    </div>
    ...
    <div class="row" style="top: 40px; position: inherit;" id="form-cliente">
                ...
                <div class="card-body">
                        ...
                        <div class="modal-dialog" :class="modal_size" role="document">
                            ...
                            <div class="modal-content" v-if="methods == 'create'">
                                <div class="modal-header" style="padding: 10px 15px">
                                    <h5 class="mb-0">NUEVO <span style="color: #929292; font-size: 17px; font-weight: 400;">(CLIENTE)</span></h5>
                                ...
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-10">
                                            <label for="numero_documento" class="label-sm">RUC / DNI</label>
                                            <div class="input-group">
                                                <div class="input-group-prepend">
                                                    <button class="btn btn-info" type="button" v-on:click="Reniec"><i class="fas fa-search"></i> Reniec</button>
                                                </div>
                                                <input type="text" id="numero_documento" v-model="numero_documento" class="form-control" :class="[errors.numero_documento ? 'is-invalid' : '']" :readonly="loading"
                                                maxlength="11" onkeypress="if (event.keyCode < 48 || event.keyCode > 57) { event.returnValue = false}" v-on:keyup.enter="Sunat">
                                                <div class="input-group-append">
                                                    <button class="btn btn-dark" type="button" v-on:click="Sunat"><i class="fas fa-search"></i> Sunat</button>
                                                </div>
                                            </div>
                                        ...
                                        <div class="form-group col-md-6">
                                            <label for="telefono" class="label-sm">CELULAR / TELEFONO</label>
                            ...
                            <div class="modal-content" v-if="methods == 'edit'">
                                <div class="modal-header" style="padding: 10px 15px">
                                    <h5 class="mb-0">EDITAR <span style="color: #929292; font-size: 17px; font-weight: 400;">(CLIENTE)</span></h5>
                                    <button type="button" title="Cerrar" data-dismiss="modal" aria-label="Close" v-on:click="closeModal(methods)" class="btn btn-danger btn-xs float-right" style="padding: 0px 7px;">X</button>
                                </div>
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-8">
                                            <label for="numero_documento" class="label-sm">@{{tipo_documento}}</label>
                                            <input type="text" id="numero_documento" v-model="numero_documento" class="form-control" readonly>
                                        </div>
                                        <div class="form-group col-md-12">
                                            <label for="nombres" class="label-sm">NOMBRES O RAZÓN SOCIAL</label>
                                            <input type="text" id="nombres" v-model="nombres" class="form-control" readonly>
                                        </div>
                                        <div class="form-group col-md-12">
                                            <label for="direccion" class="label-sm">DIRECCIÓN</label>
                                            <input type="text" id="direccion" v-model="direccion" class="form-control" :class="[errors.direccion ? 'is-invalid' : '']" :readonly="loading">
                            ...
                            <div class="modal-content" v-if="methods == 'delete'">
                                <div class="modal-header" style="padding: 10px 15px">
                                    <h5 class="mb-0">ELIMINAR <span style="color: #929292; font-size: 17px; font-weight: 400;">(CLIENTE)</span></h5>
                                    <button type="button" title="Cerrar" data-dismiss="modal" aria-label="Close" v-on:click="closeModal(methods)" class="btn btn-danger btn-xs float-right" style="padding: 0px 7px;">X</button>
                                </div>
                                <div class="modal-body" style="padding: 15px 15px;">
                                    <p class="text-center">
                                        Realmente desea eliminar al Cliente <strong>"@{{nombres}}"</strong>
                                    </p>
                                </div>
                                <div class="modal-footer" style="padding: 10px 15px;">
                    ...
                    <div class="row">
                        <div class="mb-3 mt-3 col-md-9">
                            ...
                            data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('', null, 'create', null)">
                            ...
                            data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('', active, 'edit', seleccion)">
                            ...
                            <button type="button" class="btn btn-icon btn-danger mr-2" style="min-width: 88px;" v-if="(active != 0) && (seleccion.get_soportes_count == 0)"
                            data-bs-toggle="modal" data-bs-target="#formularioModal" v-on:click="formularioModal('modal-sm', active, 'delete', seleccion.nombres)">
                        ...
                        <div class="mb-3 mt-3 col-md-3">
                            <div class="p-b-10">
                                <input type="text" class="form-control" id="seacrh" v-model="search" placeholder="Nombres del Clientes." v-on:keyup.enter="Buscar">
                            </div>
                        ...
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="cell-2">N° Documento</th>
                                    <th class="cell-3">Datos Completos</th>
                                    <th class="cell-4 text-center">Tipo Documento</th>
                            ...
                            <tbody>
                                ...
                                <template v-if="listTable">
                                    <template v-if="listaRequest.length != 0">
                                        <tr v-for="(cliente, index) in listaRequest" :class="{ activado : active == cliente.id }" v-on:click="Fila(cliente.id, cliente)" style="cursor: pointer;">
                                            <td class="text-center">@{{(index + pagination.index + 1)}}</td>
                                            <td><strong>@{{cliente.tipo}} : </strong>@{{cliente.id}}</td>
                                            <td>
                                                <div><i class="fas fa-user"></i> @{{cliente.nombres}}</div>
                                                <div>
                                                    <i class="fas fa-phone-square"></i> @{{cliente.celular}} &nbsp;| &nbsp;
                                                    <i class="fas fa-envelope"></i> @{{cliente.email}}
                                                </div>

                                            </td>
                                            <td class="text-center">@{{cliente.tipo}}</td>
                                            <td class="text-center">@{{Fecha(cliente.updated_at)}}</td>
...
@section('js')
    <script src="{{asset('js/views/clientes/cliente.js')}}"></script>
@endsection
